"""Transport-neutral core: request/response models + ``_do_*`` handlers.

This module carries the whole feature surface (12 operations: check, explain,
validate_cross_repo, query, audit, propose, diff, narrate, validate, staleness,
impact, freshness) **without** importing FastMCP. The CLI imports from here, so
``pip install iil-adrfw`` stays lean; ``iil_adrfw.server`` adds the MCP transport
on top and needs the ``[mcp]`` extra.
"""
...
def _adrs_dir() -> Path:
    # `docs/adr` is the layout every ADR-carrying repo in the fleet uses. The
    # previous default (`./adrs`) matched none of them, so an unset env var
    # silently yielded an empty constitution that still reported perfect health.
    return Path(os.environ.get("IIL_ADRFW_ADRS_DIR", "docs/adr")).resolve()


def _schemas_dir() -> Path:
    override = os.environ.get("IIL_ADRFW_SCHEMAS_DIR")
    return Path(override).resolve() if override else get_schema_dir()
...
def _do_check(req: CheckRequest) -> CheckResponse:
    ...
    # Imported lazily so a `validate`/`audit`-only consumer never loads libcst.
    # `build_checker` raises an actionable RuntimeError if the `[checkers]` extra
    # is missing and a rule actually needs the AST checkers.
...
def _do_audit(req: AuditRequest) -> AuditResponse:
    ...
    # Tolerant load: an audit is a report about the whole constitution, so one
    # malformed ADR must not abort it. Before this, a single invalid file made
    # `audit` die with exit 3 ("internal error"), which made the tool unusable in
    # precisely the repos whose ADR hygiene most needed auditing (meiki-hub: 23
    # of 36 ADRs invalid). Unloadable files become findings, not a crash.
    load_errors: list[dict] = []
    adrs = load_adrs(_adrs_dir(), _schemas_dir(), validate=True, errors=load_errors)
    graph = ConstitutionGraph.build(adrs)
    ...
    unloadable = [
        FindingOut(
            auditor="loadability",
            severity="error",
            affected_adrs=[err["file"]],
            description=f"ADR could not be loaded and was excluded from the audit: {err['error']}",
            proposed_resolution="Fix the frontmatter so the ADR validates (`iil-adrfw validate`).",
            evidence=[err["file"]],
        )
...
def _do_impact(req: ImpactRequest) -> ImpactResponse:
...
def _do_freshness(req: FreshnessRequest) -> FreshnessResponse:
